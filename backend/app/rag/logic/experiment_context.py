...
# chunking は split_for_rag_with_metadata 一本に統一。
# 全ロジックモジュールが [{"text": str, "metadata": dict}, ...] 形式で返す。
# -----------------------------------------------------------------------------
...
def get_split_for_rag_with_metadata() -> Callable[..., list[dict[str, Any]]]:
    custom = _split_for_rag_with_metadata_var.get()
    if custom is not None:
        return custom
    from app.rag.logic.chunking import split_for_rag_with_metadata

    return split_for_rag_with_metadata
