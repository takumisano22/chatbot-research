...
#        -> _demote_standalone_h4_prefixes_to_bullets()  ← metadata 後、ひ孫無しのみ
...
def _split_at_block_boundary(
    ...
    prefix = ("\n".join(prefix_lines).rstrip() + "\n") if any(
        pl.strip() for pl in prefix_lines
    ) else ""
...
# 孫境界の "#### " は _format_subtree / 分割まで維持し、metadata 確定後にだけ変換する。
# ひ孫で "- " が既に混ざるチャンクはそのまま（判別用の #### を残す）。
def _demote_standalone_h4_prefixes_to_bullets(text: str) -> str:
    if "####" not in text or _chunk_has_markdown_list_dash(text):
        return text
    return re.sub(r"(^|\n)#### ", r"\1- ", text)


# ChromaDB へ渡す最終本文だけ Markdown 記法を外す。
# 構造判定と分割境界では Markdown を使い続けるため、flatten 後の出力直前でのみ適用する。
def _strip_output_markdown_prefixes(text: str) -> str:
    return re.sub(r"(?m)^(?:#{1,4}|-) ", "", text)
...
# logic_05 は内部の構造認識では Markdown を維持し、返却チャンク本文だけ平文化する。
def split_for_rag_with_metadata(
    *, text: str, chunk_size: int = 800, chunk_overlap: int = 0
    ...
    for c in flatten_chunks(root, config):
        plain_text = _strip_output_markdown_prefixes(c["text"])
        if plain_text.strip():
            chunks.append({"text": plain_text, "metadata": c.get("metadata", {})})
    return chunks
