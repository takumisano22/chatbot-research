...
# reranking_logic_02.py 修正依頼プロンプト

## 役割
あなたは、再ランキングロジックのコード改善をサポートするアシスタントです。

## 目的
現在は、max * ratioでの足きりと、子チャンクの親チャンクへの統合を行っています。
これらを、averageでの足きりに変更することと、親チャンクへの統合条件を厳しくしたいです。
意図としては、子チャンクの文脈を補強することと、推論LLMに渡すコンテキストを太らせすぎないことを両立するための調整がしたいという内容です。

## 要件
- 現在、threshold と max_score を利用して項目を減らしている箇所を削除してください。
- ロジックの初期段階（new_topKを定めた直後）で、取得した元データの平均値をとってください。
- items = _promote_orphan_grandchildren(items) と、二度目の items = _promote_score(~)　の間で、前項で取得した平均値以下のスコアをもつアイテムを削除してください。
- その他のロジックは、基本変更しなくてよいです。整合性に問題がある場合だけ検討してください。

## 注意点
- KISS・YAGNI原則に則り必要最小の実装を行ってください。
- 判断に迷う箇所は、対話的に実装を行ってください。
- 実装の前に、設計を行って検討を完了してから、実装を行ってください。
- 適宜コメントを修正してください。
- docs/structure-aware_logic06_rerank02_guide.mdも変更内容に合わせて適宜修正を行ってください。