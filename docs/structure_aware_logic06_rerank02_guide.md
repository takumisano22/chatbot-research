# chunking_logic_06 / reranking_logic_02 説明書
...
`reranking_logic_02.py` は、`chunking_logic_06.py` が付与した階層 metadata を使い、ベクトル検索で取得した候補を LLM に渡しやすい件数・粒度へ圧縮するロジックです。親・子・孫チャンクの重複を避けつつ、下位チャンクの高いスコアを上位チャンクへ引き継ぎます。
...
| `backend/app/rag/logic/reranking/reranking_logic_02.py` | 階層 metadata ベースの候補圧縮 |
...
`reranking_logic_02.py` の目的は、ベクトル検索で拾った候補を「LLM に渡す文脈」として適切な粒度へ寄せることです。
...
そのため rerank では、単純にスコア順で上位を切るのではなく、階層 metadata を見て重複を整理します。下位チャンクが強くヒットした場合は、そのスコアを代表する上位チャンクへ引き継ぎ、最終的には少数の読みやすい候補へ圧縮します。
...
rerank
  -> 件数が 4 未満ならそのまま返す
  -> new_top_k = max(4, top_k // 10)
  -> 元候補の平均スコアを保持
  -> final_score を作業スコアとして保持
  -> 同一 chunk_id を最高スコアへ集約
  -> parent / child / grandchild の重複を段階的に削除・昇格
  -> 元候補の平均スコア以下を削除
  -> 複数子を親へ集約
  -> スコア順で new_top_k 件を返す
```

`top_k=30` の research_pair では `new_top_k = max(4, 30 // 10) = 4` になります。

#### 1. 少数候補はそのまま返す

候補数が `_MIN_TOP_K = 4` 未満なら、再ランキングをスキップします。少数候補に対して平均カットや階層集約をかけると、LLM に渡る文脈が薄くなりすぎるためです。
...
この残った孫を `reranking_logic_02.py` では「孤立孫」として扱い、`parent_chunk_id` をキーに親へスコアを引き継いで削除します。
...
最初に保持した元候補の全体平均 `avg_score` 以下の候補を削除します。この足切りは孤立孫の親引き継ぎ後、複数子を親へ集約する前に行います。

親集約前に平均以下を落とすことで、弱い子チャンクを根拠に親チャンクへ広げるケースを抑えます。

#### 7. 複数子を親へ集約する

同じ `parent_chunk_id` に `child` が 2 件以上残り、かつ `parent` も存在する場合、子の最高スコアを代表 `parent` へ引き継ぎ、`child` を削除します。

複数の子が同じ章配下でヒットしているなら、章全体を代表として渡す方が LLM 文脈として読みやすい、という判断です。

平均以下の足切り後に判定するため、親へ集約されるのは平均を上回る子が複数残った場合に限られます。

#### 8. 上位件数へ圧縮

スコア降順に並べ、`new_top_k` 件までに絞ります。残数が `new_top_k` 未満なら、返却する `new_top_k` も残数に合わせます。

### 互換性

`chunk_role`、`parent_chunk_id`、`child_chunk_id` など必要 metadata がない候補は、階層グループ化の対象外として通します。非構造化チャンクや他の chunking logic 由来の候補でも、同一 `chunk_id` の集約、平均カット、TopK 圧縮だけが適用される設計です。

---

## まとめ

`chunking_logic_06.py` は「構造を推定して、親・子・孫の検索粒度を作る」ロジックです。`reranking_logic_02.py` は「検索結果に混ざった複数粒度の候補を、metadata で代表候補へまとめる」ロジックです。

この 2 つは、`parent_chunk_id` / `child_chunk_id` / `grandchild_chunk_id` と `chunk_role` を契約としてつながっています。チャンク化側で階層 ID を正しく付け、再ランキング側でその ID を使って重複を整理する、という分担です。
