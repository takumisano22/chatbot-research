...
| `outputs/` | 実験 CSV の出力先（CLI が UTC タイムスタンプ付きファイル名で保存）。 |
...
事前に `.env`（ホストの LLM / Langfuse / `VECTOR_STORE_SERVER_HOST` 等）を整え、`research_pairs` / `qa_datasets` / `ingest_document` に実データを配置してください。CSV は `outputs/` にマウントされたホスト側ディレクトリに残ります。
